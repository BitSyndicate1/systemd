...
enum nss_status userdb_getgrnam(
        ...
        r = groupdb_by_name(NULL, name, /* match= */ NULL, nss_glue_userdb_flags()|USERDB_SUPPRESS_SHADOW, &g);
        if (r < 0 && r != -ESRCH) {
                *errnop = -r;
                return NSS_STATUS_UNAVAIL;
        }
...
enum nss_status userdb_getgrgid(
        ...
        r = groupdb_by_gid(NULL, gid, /* match= */ NULL, nss_glue_userdb_flags()|USERDB_SUPPRESS_SHADOW, &g);
        if (r < 0 && r != -ESRCH) {
                *errnop = -r;
                return NSS_STATUS_UNAVAIL;
        }
...
enum nss_status userdb_getsgnam(
        ...
        r = groupdb_by_name(NULL, name, /* match= */ NULL, nss_glue_userdb_flags(), &hr);
        if (r == -ESRCH)
                return NSS_STATUS_NOTFOUND;
        if (r < 0) {
                *errnop = -r;
