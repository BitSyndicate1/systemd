...
static int uid_is_available(
        ...
        r = groupdb_by_gid(NULL, candidate, /* match= */ NULL, USERDB_AVOID_MULTIPLEXER, /* ret= */ NULL);
        if (r >= 0)
                return false;
        if (r != -ESRCH)
                return r;
...
static int name_is_available(
        ...
        r = groupdb_by_name(NULL, user_name, /* match= */ NULL, USERDB_AVOID_MULTIPLEXER, /* ret= */ NULL);
        if (r >= 0)
                return false;
        if (r != -ESRCH)
                return r;
